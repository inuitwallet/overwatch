...
class CloudWatchLogsConsumer(SyncConsumer):
    def get_cloudwatch_logs(self, event):
        ...
        async_to_sync(get_channel_layer().group_send)(
            "cloudwatch_logs_{}".format(bot.pk), {"type": "logs.clear",}
        )
    ...
    def stream_events(self, bot_pk, client, group, stream, next_token=None):
        if next_token:
            ...
            events = client.get_log_events(
                logGroupName=group, logStreamName=stream, startFromHead=False, limit=50
            )
        ...
        return self.stream_events(
            bot_pk, client, group, stream, events.get("nextForwardToken")
        )
