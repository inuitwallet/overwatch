...
class Bot(object):
    ...
    def get_exchange_wrapper(self):
        ...
        self.market = next(
            m for m in self.wrapper.fetch_markets() if m.get("symbol") == self.symbol
        )
    ...
    def get_prices(self):
        ...
        pm = PriceManager()
        self.quote_price = pm.get_price(
            self.market.get("quote"), self.config.get("quote_price_url")
        )
        self.base_price = pm.get_price(
            self.market.get("base"), self.config.get("base_price_url")
        )
        ...
        if self.config.get("use_market_price"):
            ...
            if (
                self.config.get("peg_currency", "peg").upper()
                == self.market.get("quote", "quote").upper()
            ):
                known_config = True
                # this is option 1
            ...
            if (
                self.config.get("peg_currency", "peg").upper()
                == self.market.get("base", "base").upper()
            ):
                known_config = True
                # this is option 2
        ...
        self.buy_price = float(price) - (
            float(self.config.get("fee") + self.config.get("bid_spread")) * float(price)
        )
        self.sell_price = float(price) + (
            float(self.config.get("fee") + self.config.get("ask_spread")) * float(price)
        )
    ...
    def check_amount(self, amount):
        ...
        if amount_max:
            if amount > amount_max:
                self.logger.warning(
                    "Maximum order amount breached: {} > {}".format(amount, amount_max)
                )
                return False
    ...
    def check_price(self, price):
        ...
        if price_min:
            if price < price_min:
                self.logger.warning(
                    "Minimum order price not reached: {} < {}".format(price, price_min)
                )
                return False
        ...
        if price_max:
            if price > price_max:
                self.logger.warning(
                    "Maximum order price breached: {} > {}".format(price, price_max)
                )
                return False
    ...
    def check_cost(self, amount, price):
        ...
        if cost_min:
            if cost < cost_min:
                self.logger.warning(
                    "Minimum order cost not reached: {} < {}".format(cost, cost_min)
                )
                return False
        ...
        if cost_max:
            if cost > cost_max:
                self.logger.warning(
                    "Maximum order cost breached: {} > {}".format(cost, cost_max)
                )
                return False
    ...
    def check_existing_side_orders(self, orders, order_type, price):
        ...
        for order in orders:
            ...
            self.logger.info(
                "Checking {} order {}".format(order_type.title(), order.get("id"))
            )
    ...
    def check_side_total_is_reached(self, side, side_total, price):
        ...
        if total < target:
            ...
            check_currency = (
                self.market.get("quote") if side == "buy" else self.market.get("base")
            )
            balance = self.get_available_balance(check_currency)

            self.logger.info(
                "Got available balance of {} {}".format(balance, check_currency)
            )
            ...
            self.logger.info(
                "Got working balance of {} {}".format(balance, self.market.get("base"))
            )
            ...
            if balance < difference:
                ...
                self.logger.warning(
                    ...
                    "but only {:.4f} available".format(difference, target, balance)
                )
                vigil_alert(
                    alert_channel_id=os.environ["VIGIL_FUNDS_ALERT_CHANNEL_ID"],
                    data={
                        "bot_name": self.name,
                        "currency": self.market.get("base")
                        if side == "buy"
                        else self.market.get("quote"),
    ...
    def check_orders_over_target(self):
        orders = self.get_open_orders()
        self.check_orders_over_target_side(
            orders, "buy", self.total_bid, self.buy_price
        )
        self.check_orders_over_target_side(
            orders, "sell", self.total_ask, self.sell_price
        )
    ...
    def check_orders_over_target_side(self, orders, side, side_total, price):
        ...
        self.logger.info(
            "{}: Total = {}, Target = {}".format(side.title(), total, target)
        )
        ...
        if total > target:
            ...
            self.logger.info(
                "Got Diff of {}. Removing {} orders".format(difference, num)
            )
            ...
            for order in remove_orders[:num]:
                self.logger.info(
                    "Cancelling {} Order {}".format(side.title(), order.get("id"))
                )
                self.wrapper.cancel_order(order.get("id"))
    ...
    def get_trades(self):
        ...
        for trade in sorted(
            self.wrapper.fetch_my_trades(self.symbol),
...
def main(event, context):
    Bot(os.environ.get("BOT_NAME"), os.environ.get("EXCHANGE"),).run()
