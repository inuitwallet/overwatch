...
class Overwatch(object):
    ...
    def generate_hash(self):
        ...
        return (
            ...
            hmac.new(
                ...
                "{}{}{}".format(self.name.lower(), self.exchange.lower(), nonce).encode(
                    "utf-8"
                ),
                hashlib.sha256,
            ).hexdigest(),
    ...
    def handle_response(self, r):
        if r.status_code != requests.codes.ok:
            self.logger.error(
                "overwatch gave a bad response code: {}".format(r.status_code)
            )
            return False
    ...
    def get_config(self):
        ...
        if not config:
            self.logger.error(
                "unable to get config for {}@{}".format(self.name, self.exchange)
            )
            return False
    ...
    def record_price(
        self, price, bid_price, ask_price, market_price, base_price, quote_price
    ):
        nonce, generated_hash = self.generate_hash()
        self.handle_response(
            requests.post(
    ...
    def record_balances(
        self, bid_available, ask_available, bid_on_order, ask_on_order, unit
    ):
        nonce, generated_hash = self.generate_hash()
        self.handle_response(
            requests.post(
